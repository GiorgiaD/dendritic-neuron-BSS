...
if __name__ == '__main__':
    ...
    n_rep_pretrain = args.n_rep_pretrain
    ...
    for sim_n in range(n_repet):
        ...
        # extract and concatenate FIRST THE TARGETS and then the different mixtures
        source = np.zeros((n_in,0))
        mix_len = []
        pre_source = np.zeros((n_in,0))
        ...
        for idx in range(n_sounds_tot):
            ...
            pre_source = np.concatenate((pre_source,mel),axis = 1)
            mix_len.append(np.shape(mel)[1])
        plt.figure()
        librosa.display.specshow(pre_source, x_axis='time', y_axis='mel')
        plt.colorbar()
        plt.title('Mel frequency spectrogram for the pre source')
        plt.savefig(save_path+exp_name+'_'+str(sim_n)+'_pre_source.pdf', fmt='pdf', dpi=350)    
            
        pre_source = np.tile(pre_source,n_rep_pretrain)
        pre_source_len = np.shape(pre_source)[1]
        print('Len of pre source is ',pre_source_len)
        
        if all_comb==False:
            for idx in range(n_mixt):
                print('combining: target = {} ; idx = {}'.format(n_sounds_list[0],n_sounds_list[idx+1]))
                mel = (dataset[n_sounds_list[0]]+dataset[n_sounds_list[idx+1]])/2
                mel = mel.flatten()
                mel = np.expand_dims(mel, axis=1) 
            ...
            for idx_i,sound_i in enumerate(n_sounds_list):
                for idx_j,sound_j in enumerate(n_sounds_list[idx_i+1:]):
                    ...
                    mel = (dataset[sound_i]+dataset[sound_j])/2
                    mel = mel.flatten()
                    mel = np.expand_dims(mel, axis=1)
                    source = np.concatenate((source,mel),axis = 1)
                    mix_len.append(np.shape(mel)[1])
        ...
        print('Len of source is ',source_len)
        # concatenate the pre_source and the source
        source = np.concatenate((pre_source,source),axis = 1)
        source_len = np.shape(source)[1]
        print('Len of final source is ',source_len)
